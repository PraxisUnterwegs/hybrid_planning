...
def generate_common_hybrid_launch_description():
    ...
    planning_pipelines_config = {
        ...
        "request_adapters": """default_planner_request_adapters/AddTimeOptimalParameterization default_planner_request_adapters/FixWorkspaceBounds default_planner_request_adapters/FixStartStateBounds default_planner_request_adapters/FixStartStateCollision default_planner_request_adapters/FixStartStatePathConstraints""",
        "start_state_max_bounds_error": 0.1,
        "collision_clearance": 0.3,  # 设置碰撞清除距离
        "collision_threshold": 3.0,  # 设置碰撞阈值
        "smoothness_cost_weight": 0.01,  # 设置平滑成本权重
        "obstacle_cost_weight": 2.0,  # 设置障碍物成本权重
        "learning_rate": 0.01,  # 设置学习率
        "max_iterations": 200,  # 设置最大迭代次数
        "max_iterations_after_collision_free": 5,  # 设置无碰撞后的最大迭代次数
        "ridge_factor": 0.001,  # 设置岭因子
        "animate_path": False,  # 是否动画显示路径
        "add_randomness": True,
        "random_jump_amount": 0.5,
        "enable_failure_recovery": True,
        "joint_update_limit": 0.2,
        "smoothness_cost_acceleration": 0.5,
    	}
    }
    ...
    chomp_planning_yaml = load_yaml(
   	"moveit_resources_panda_moveit_config", "config/chomp_planning.yaml"
    )
    planning_pipelines_config["chomp"].update(chomp_planning_yaml)
